...
export function Sidebar({ logout }) {
    ...
    return (
        <div className="flex min-h-dvh max-h-full w-full ">
            <div className={`flex-h-grow ${open ? "lg:w-72  lg:pb-72 pb-36 w-screen bg-slate-50 " : "lg:w-28  w-16 lg:pb-96 lg:bg-slate-50 "} transition-width lg:bg-slate-50 lg:p-5 lg:pt-8 relative ease-in-out duration-300 lg:duration-150`}>
            <img
                    src={controlImage}
                    className={`absolute cursor-pointer -right-5 top-9  border-slate-100 border-5 rounded-full 
                        ${!open && "rotate-180"} 
