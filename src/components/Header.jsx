    ...
    <nav className="flex flex-row justify-between h-20 bg-white relative pt-2 pb-2 ">
      ...
      <div className="lg:hidden cursor-pointer justify-between"
          onClick={() => setMenuOpen(!menuOpen)}>
            <img
              src={menuIcon}
                className={`${menuOpen ? "w-20 h-20" : "w-20 h-20"} p-5 mr-1 pt-2 pb-8 relative ease-in-out duration-300 flex rounded-full 
                  ${!menuOpen && "scale-x-[-1]"}`}
                onClick={() => setMenuOpen(!menuOpen)}
        ...
        <div className="flex lg:justify-end lg:space-x-1 lg:flex-row lg:mt-0 mt-3 ">  
          <li className="lg:p-0 px-10 py-4 lg:bg-transparent bg-slate-200">
            <div className="flex items-center lg:m-0 ml-1 lg:text-xl lg:mx-16 text-center ">
              ...
              <span className="leading-none text-gray-600 text-sm mx-2 lg:mx-7 text-center">En línea</span>
                <Link to="/Settings" className="px-4 py-1.5 ml-5 text-sm text-center font-medium rounded-2xl lg:mx-7 bg-gray-100 text-gray-600 hover:bg-slate-200 hover:text-sky-800 transition duration-200">
                  Perfil de usuario
                </Link> 
            </div>
          </li>
